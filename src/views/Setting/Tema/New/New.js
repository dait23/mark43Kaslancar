...
class NewTema extends Component {
  ...
  handlePost = async () => {
    ...
    await this.props.addTema({variables: {name, slug, imageUrl, imageId}})

   //history.push('#/slider/all');
   //history.goBack;
   window.location= MainLink + "setting/tema/all";
   //window.location.reload(true);
  }

}
const addMutation = gql`
  mutation addTema($name: String!, $slug: String!, $imageUrl:String!, $imageId: String) {
    createTema(name: $name, slug: $slug, imageUrl: $imageUrl, imageId: $imageId) {
      id
      name
      slug
      imageUrl
...
const PageWithMutation = graphql(addMutation, { name: 'addTema' })(NewTema)
