...
class NewTipe extends Component {
  ...
  handlePost = async () => {
    ...
    await this.props.addTipe({variables: {name, slug, imageUrl, imageId}})

   //history.push('#/slider/all');
   //history.goBack;
   window.location= MainLink + "setting/tipe/all";
   //window.location.reload(true);
  }

}
const addMutation = gql`
  mutation addTipe($name: String!, $slug: String!, $imageUrl:String!, $imageId: String) {
    createTipe(name: $name, slug: $slug, imageUrl: $imageUrl, imageId: $imageId) {
      id
      name
      slug
      imageUrl
...
const PageWithMutation = graphql(addMutation, { name: 'addTipe' })(NewTipe)
